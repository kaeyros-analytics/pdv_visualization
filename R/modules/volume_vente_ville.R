...
volume_vente_ville_ui <- function(id) {
  ns <- NS(id)
  shinycssloaders::withSpinner(plotly::plotlyOutput(outputId = ns("volume_vente_ville"), height = "500px"),
                               type = 8, color = 'grey')
} # end top_service_type_ui

volume_vente_ville_server <- function(input, output, session) {
  
  output$volume_vente_ville <- renderPlotly({
    
    if(input$pdvcity == "All"){ df_bavaria_test2 <- df_bavaria1 %>% 
